...
const {
    ...
    loadActiveMatches, addToQueue, removeFromQueue,
    createMatch, removeActiveMatch, returnToQueue
} = require("./matchmaking");
...
async function testTelegramConn() {
    return new Promise((resolve) => {
        console.log("Testando conectividade com api.telegram.org...");
        const req = https.get("https://api.telegram.org", (res) => {
            console.log(`Status da conexão: ${res.statusCode}`);
            resolve(true);
        });
        req.on("error", (e) => { console.error(`ERRO DE REDE: ${e.message}`); resolve(false); });
        req.setTimeout(10000, () => { console.error("TIMEOUT Telegram."); req.destroy(); resolve(false); });
    });
}
...
const lobbyMessages = new Map(); // id -> { chatId, messageId }
...
async function markOnline(id) {
    ...
    if (!wasOnline) await broadcastLobbyUpdate(id);
}
...
function getLobbyMatchText(match, playerId, playerDetails) {
    ...
    text += `🆔 ID DA PARTIDA\n`;
    text += `   #${match.match_id}  |  Modo: ${match.mode === "3v3" ? "3v3 Casual" : "5v5 Ranked"}\n`;
    text += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    ...
    text += `✅ CONFIRMAÇÕES\n`;
    text += `   ${dots}  (${confirmedCount}/${totalCount})`;
...
function getCountdownText(count) {
    ...
    let text = `⏱ BUSCA INICIANDO EM\n`;
    text += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    text += `          ${count}\n\n`;
    text += `   ${bar}\n`;
    text += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    text += `⚡ Preparem-se para buscar`;
    return text;
}

function getResultRequestText(matchId, team) {
    let text = `🏁 FIM DE PARTIDA\n`;
    text += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    text += `🆔 ID DA PARTIDA\n`;
    text += `   #${matchId}\n\n`;
    text += `👤 SEU TIME\n`;
...
function getRankingText(rows) {
    ...
    rows.forEach((p, i) => {
        ...
        const nick = p.nickname.padEnd(14);
        text += `${prefix} ${nick}  ${fc}  ${p.games}\n`;
    });

    text += `━━━━━━━━━━━━━━━━━━━━━━\n`;
    text += `   Mín. 10 partidas para rankear`;
    return text;
}
...
function getPenaltyText(nick, days) {
    ...
    text += `   Motivo    Não confirmou\n`;
    text += `   Duração   ${days} dias\n`;
    text += `   Derrota   +2 registradas\n`;
    text += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    text += `📊 FC ATUAL\n`;
    text += `   💀 BLOQUEADO\n\n`;
    text += `📅 EXPIRA EM\n`;
...
function getRemakeText() {
    return `♻️ REMAKE DETECTADO\n━━━━━━━━━━━━━━━━━━━━━━\n\nPartida encerrada em menos de 7 minutos.\nHistórico não será afetado.`;
}

// ─── BOT ──────────────────────────────────────────────────────────────────────

const bot = new Telegraf(process.env.BOT_TOKEN, { handlerTimeout: 90_000 });
...
async function refreshLobbyForUser(userId) {
    ...
    try {
        ...
        await bot.telegram.editMessageText(msgRef.chatId, msgRef.messageId, undefined, text, { ...keyboard });
    } catch (err) {
...
bot.action("JOIN_3V3", async (ctx) => {
    ...
    try {
        const text = await getStatusText(ctx.from.id);
        await ctx.editMessageText(text, getLobbyKeyboard(ctx.from.id));
...
const lobbyMatchMessages = new Map(); // matchId -> [{userId, chatId, messageId}]
...
async function checkQueue(mode) {
    ...
    for (const player of allPlayers) {
        ...
        const msg  = await bot.telegram.sendMessage(
            player.id, text,
            Markup.inlineKeyboard([[Markup.button.callback("✅ Nick alterado", `CONFIRM_${match.match_id}`)]])
        );
    ...
    // Timeout de 60s para confirmação
    setTimeout(async () => {
...
bot.action(/CONFIRM_(.+)/, async (ctx) => {
    ...
    // Atualiza mensagem de todos no lobby
    ...
    for (const ref of msgRefs) {
        try {
            await bot.telegram.editMessageText(
                ...
                getLobbyMatchText(match, ref.userId, playerDetails),
                Markup.inlineKeyboard([[Markup.button.callback("✅ Nick alterado", `CONFIRM_${match.match_id}`)]])
            );
        } catch (_) {}
    }
    ...
    for (const p of players) {
        const msg = await bot.telegram.sendMessage(p.id, getCountdownText(10));
    ...
    for (let i = 9; i >= 1; i--) {
        ...
        for (const msg of cdMsgs) {
            await bot.telegram.editMessageText(msg.chatId, msg.messageId, undefined, getCountdownText(i)).catch(() => {});
        }
    }
    await sleep(1000);
    for (const msg of cdMsgs) {
    ...
    // Solicita resultado após 3 min
    setTimeout(async () => {
        for (const player of players) {
            const team = match.teamA.find(p => p.id === player.id) ? "A" : "B";
            await bot.telegram.sendMessage(
                player.id,
                getResultRequestText(match.match_id, team),
...
async function finalizeMatch(match) {
    ...
    if (isRemake) {
        for (const p of players) await bot.telegram.sendMessage(p.id, getRemakeText());
    } else if (!hasConflict) {
...
bot.command("perfil", async (ctx) => {
    ...
    await ctx.reply(getProfileText(player));
    await sleep(1000);
    await showMainMenu(ctx);
});
...
bot.command("ranking", async (ctx) => {
    ...
    await ctx.reply(getRankingText(rows));
    await sleep(1000);
    await showMainMenu(ctx);
});
...
bot.command("changenick", async (ctx) => {
    ...
    const [existing] = await pool.execute(
        "SELECT * FROM players WHERE nickname = ? AND telegram_id != ?", [newNick, ctx.from.id]
    );
...
bot.command("relatorio", async (ctx) => {
    ...
    if (!matchId) {
        const [conflicts] = await pool.execute(
            "SELECT id, created_at FROM matches WHERE conflict_report IS NOT NULL ORDER BY id DESC LIMIT 10"
        );
...
bot.action(/PENALIZE_(.+)_(.+)/, async (ctx) => {
    ...
    if (pRows.length > 0) {
        ...
        await bot.telegram.sendMessage(Number(targetId), getPenaltyText(cur.nickname, penalDays));
    }
        ...
        const launchBot = async (retries = 5) => {
            for (let i = 0; i < retries; i++) {
                try {
                    ...
                    console.error(`Falha ${i + 1}:`, err.message);
